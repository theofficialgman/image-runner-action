...
tempdir=${RUNNER_TEMP:-/home/actions/temp}/image-runner
if [[ ! -f "${tempdir}/image-runner.img" ]]; then
    rm -rf ${tempdir}
    mkdir -p ${tempdir}
    cd ${tempdir}
    ...
    if compgen -G $PATTERN > /dev/null; then
        cat l4t.* > image-runner.img
    else
        mv "$(ls *.img */*.img 2>/dev/null | head -n 1)" image-runner.img
    fi
fi
cd ${tempdir}
find . -type f -not -name 'image-runner.img' -delete
ls -l ${tempdir}
echo "image=${tempdir}/image-runner.img" >> "$GITHUB_OUTPUT"
